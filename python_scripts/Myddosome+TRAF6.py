def oligomer (n, rlc, traf6, p1, p2, p3, p4, p5, p6):
   ...
   # evaluate the Receptor array: count ones and zeros. Gives a single number for each state. 
    ...
    t = traf6 [rs] #we look at the same receptor as before and will calculate the probability of TRAF6 association
    ...
    #we look at the same receptor as before and will calculate the probability of TRAF6 association
    ...
    elif s >= 6 and t == 0:          # If receptor 's' is bound by more than 6 monomers and no TRAF6 is associated
        if r3 <= p5:
            traf6 [rs] +=1        # increase bound TRAF6 by one TRAF6-monomer with probability p5
...
def oligomer_time (step, sec, n, rlc_0, traf6_0, p1, p2, p3, p4, p5, p6):
    ...
    size_list = [0 for _ in range(20)]  #set up a list to sum up the total number (cummulative number at the end of time) of complexes of certain size
    TRAF6_pos_size_list = [0 for _ in range(20)]



    for i in range (len(t_list)):
        
        free, bound, total_bound, rlc_1, TRAF6_neg, TRAF6_pos, total_bound_TRAF6, traf6_1 = oligomer (n, rlc_0, traf6_0, p1, p2, p3, p4, p5, p6)  # call the function "oligomer"
        ...
        for size in range(20):
            for site in range(len(rlc_0)):
                if rlc_0[site] == size and traf6_0[site] > 0:
...
def oligomer_time_repeat (runs, step, sec, n, rlc_0, traf6_0, p1, p2, p3, p4, p5, p6):
    ...
    for j in range (runs):
        ...
        t_list, free_list, bound_list, total_bound_list, TRAF6_neg_list, TRAF6_pos_list, total_bound_TRAF6_list, size_list, TRAF6_pos_size_list = oligomer_time(step, sec, n, rlc_0, traf6_0, p1, p2, p3, p4, p5, p6) #call the function "oligomer_time"
...
# set p1, p2 
p1 = 0.2 # probability of binding. #0.1
p2 = 0.15 # probability of dissociation. #0.2
...
t_list, free_all, bound_all, total_bound_all, TRAF6_neg_all, TRAF6_pos_all, total_bound_TRAF6_all, pct_traf6_all = oligomer_time_repeat (runs, step, sec, n, rlc_0, traf6_0, p1, p2, p3, p4, p5, p6)  # call the function "oligomer_time_repeat"
...
plt.plot(t_list, TRAF6_pos_all[0], 'orange', label='complexes with TRAF6')
plt.plot(t_list, TRAF6_neg_all[0], 'c', label='complexes w/o TRAF6')
plt.plot(t_list, mean_traf6_size[0],'darkred', label='avg TRAF6 size per complex')
...
plt.plot(t_list, mean_TRAF6_pos, 'orange', label='complexes with TRAF6')
plt.plot(t_list, mean_TRAF6_neg, 'c', label='complexes w/o TRAF6')
plt.plot(t_list, mean_traf6_size_allruns,'darkred', label='avg TRAF6 size per complex')
...
for i in range(runs-2):
    plt.plot(t_list, TRAF6_pos_all[i])
plt.plot(t_list, TRAF6_pos_all[runs-1], label='complexes with TRAF6')
plt.title('Stochastic model: {} cells: complexes with TRAF6'.format(runs))
plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.ylim(0,22)

plt.subplots_adjust(right=0.8, hspace=1.2)
plt.show()

## save the plot as a PNG file
# filename = '//data-tay/TAYLOR-LAB/Synthetic Myddosome Paper/python_scripts/Modeling_Mauriz/Figures/MyD88_TRAF6_growth.png'  # replace with your desired file path
...
plt.subplot(3,1,1)
for i in range(runs-2):
    plt.plot(s_list, pct_traf6_all[i])
plt.plot(s_list, pct_traf6_all[runs-1], label='pct of complexes with TRAF6')
plt.title('Stochastic model: {} cells: pct of complexes with TRAF6'.format(runs))
plt.xlabel('size of MyD88 complex')
plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.ylim(0,1)

plt.subplot(3,1,2)
plt.plot(s_list, mean_pct_traf6, label='pct of complexes with TRAF6')
plt.title('mean of {} cells: pct of complexes with TRAF6'.format(runs))
plt.xlabel('size of MyD88 complex')
plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.ylim(0,1)
...
# save the plot as a PNG file
filename = '//data-tay/TAYLOR-LAB/Synthetic Myddosome Paper/python_scripts/Modeling_Mauriz/Figures/MyD88_TRAF6_growth_pct_rec.png'  # replace with your desired file path
plt.savefig(filename, dpi = 300, transparent = True, bbox_inches = 'tight')