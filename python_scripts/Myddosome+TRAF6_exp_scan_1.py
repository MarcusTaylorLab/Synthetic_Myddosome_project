...
def oligomer_time_repeat_scan (runs, sub, sec_list, n, s_max, rlc_0, traf6_0, p1, p2, p3, p4, p5, p6):
    
    """Runs oligomer_time_repeat with different time values"""
    
    pct_traf6_time = [] # pct of complexes with traf6 based on complex size after each time course
    ...
    for sec in sec_list:
        ...
        pct_traf6_time.append(mean_pct_traf6)
        t_list_all.append(t_list)
        
    return pct_traf6_time, TRAF6_pos_time, bound_all_time, t_list_all
...
sec_list = np.linspace(0, 100, 21) #[1, 10, 100]
...
p5 = [0.4096933, 1.9628704, 0.4627068] #[Asym, xmid, scal] #p = Asym/(1+exp((xmid-log(input))/scal))
p6 = [0.4364759, 2.5783074, 0.6698108]
...
pct_traf6_time, TRAF6_pos_time, bound_all_time, t_list_all = oligomer_time_repeat_scan (runs, sub, sec_list, n, s_max, rlc_0, traf6_0, p1, p2, p3, p4, p5, p6)

fig, axs = plt.subplots(len(sec_list)+1,1, figsize=(8, 15))

plt.subplot(len(sec_list)+2,1,1)
for i in range(len(sec_list)):
    plt.plot(s_list, pct_traf6_time[i], label = 'pct of complexes with TRAF6; {} sec'.format(sec_list[i]))
#plt.plot(s_list, pct_traf6_time[i-1], label='pct of complexes with TRAF6')
plt.title('Parameter scan: pct of complexes with TRAF6')
plt.xlabel('size of MyD88 complex')
plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.ylim(0,1.1)

for i in range(len(sec_list)):
    plt.subplot(len(sec_list)+2,1,i+2)
    plt.plot(t_list_all[i], TRAF6_pos_time[i], label='Complexes with TRAF6; {} sec'.format(sec_list[i]))
    plt.plot(t_list_all[i], bound_all_time[i], label='bound receptors; {} sec'.format(sec_list[i]))
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    plt.ylim(0,50)
plt.xlabel('time (s)')
...
filename = '//data-tay/TAYLOR-LAB/Synthetic Myddosome Paper/python_scripts/Modeling_Mauriz/Figures/MyD88_TRAF6_time_scan.png'  # replace with your desired file path
