...
def oligomer_asymptote_repeat_scan (runs, step, sec, n, s_max, rlc_0, traf6_0, p1, p2, p3, p4, asymptote_list, p6):
    
    """Runs oligomer_time_repeat with different time values"""
    
    pct_traf6_asy = [] # pct of complexes with traf6 based on complex size after each simulation run with different p3
    TRAF6_pos_time = [] # complexes with TRAF6 over time
    bound_all_time = [] # bound receptors over time 
    
    for asymptote in asymptote_list:
        p5 = [asymptote, 2.6175046, 0.7862437] # p5 values for growth over 3 Frames #[0.4096933, 1.9628704, 0.4627068] #[Asym, xmid, scal] #p = Asym/(1+exp((xmid-log(input))/scal))
        ...
        pct_traf6_asy.append(mean_pct_traf6)
        
        #print run index and current time
        now = datetime.datetime.now()
        current_time = now.strftime("%H:%M:%S")
        print(f"Scan completed. The asymptote value is {asymptote}; Current Time =", current_time)
        
    return pct_traf6_asy, TRAF6_pos_time, bound_all_time, t_list
...
asymptote_list = np.linspace(0.1, 0.6, 6)
...
pct_traf6_asy, TRAF6_pos_time, bound_all_time, t_list = oligomer_asymptote_repeat_scan (runs, step, sec, n, s_max, rlc_0, traf6_0, p1, p2, p3, p4, asymptote_list, p6)
...
dataframes = []
for i, sublist in enumerate(pct_traf6_asy):
    ASYMPTOTE = asymptote_list[i]  # The index of the sublist
    rows = [{'PCT_RECRUITMENT': value, 'SIZE': j, 'ASYMPTOTE': ASYMPTOTE} for j, value in enumerate(sublist)]
    dataframes.append(pd.DataFrame(rows))
df = pd.concat(dataframes, ignore_index=True)
        
# # Save the DataFrame as a compressed CSV file
file_path = '//data-tay/TAYLOR-LAB/Synthetic Myddosome Paper/python_scripts/Modeling_Mauriz/Figures/df_size2_3frames_scan_asymptote.csv.gz'
