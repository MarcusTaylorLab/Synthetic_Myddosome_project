...
def oligomer_smax_repeat_scan (runs, step, sec, n, s_max_list, rlc_0, traf6_0, p1, p2, p3, p4, p5, p6):
    
    """Runs oligomer_time_repeat with different time values"""
    
    pct_traf6_smax = [] # pct of complexes with traf6 based on complex size after each simulation run with different p3
    TRAF6_pos_time = [] # complexes with TRAF6 over time
    bound_all_time = [] # bound receptors over time 
    s_list_all = []
    
    for s_max in s_max_list:
        s_list = np.linspace (0, int(s_max), int(s_max)+1)   # define the list for all sizes of oligomers; np.linspace needs an integer as input
        ...
        pct_traf6_smax.append(mean_pct_traf6)
        s_list_all.append(s_list)
        
    return pct_traf6_smax, TRAF6_pos_time, bound_all_time, t_list, s_list_all
...
s_max_list = np.linspace(25, 200, 8)
...
pct_traf6_p3, TRAF6_pos_time, bound_all_time, t_list, s_list_all = oligomer_smax_repeat_scan (runs, step, sec, n, s_max_list, rlc_0, traf6_0, p1, p2, p3, p4, p5, p6)

plt.subplot(1,1,1)
for i in range(len(s_max_list)):
    plt.plot(s_list_all[i], pct_traf6_p3[i], label = 'pct of complexes with TRAF6; {} max size'.format(s_max_list[i]))
#plt.plot(s_list, pct_traf6_time[i-1], label='pct of complexes with TRAF6')
plt.title('Parameter scan p3: pct of complexes with TRAF6')
